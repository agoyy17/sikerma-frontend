...
import axiosInstance from './axiosConfig'; // Pastikan menggunakan axiosConfig yang sudah dibuat
import './Perpanjangan.css';
import uploadIcon from "./edit.png";
...
const Perpanjangan = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Set data yang sudah difilter ke state
  ...
  const handleFileChange = (e) => {
    setFormData({ ...formData, upload_file: e.target.files[0] });  // Pastikan upload_file yang diperbarui
  };
  ...
  const handleSubmit = (e) => {
    ...
    axiosInstance.post('/extension', formDataToSend, {
      headers: {
  ...
  const handleEditFile = (e, id) => {
    ...
    const updatedEntries = entries.map((entry) =>
      entry.id === id ? { ...entry, fileName: file.name } : entry
    );
    setEntries(updatedEntries);
    alert(`File berhasil diunggah untuk Mitra ID: ${id}`);
  };
            ...
            {entries.length === 0 ? (
              ...
              entries.map((entry) => (
                <tr key={entry.id}>
                  ...
                  <td>{entry.status || 'Sedang Diproses'}</td>
                  <td>
                    <input
                      type="file"
                      accept=".doc, .docx, .pdf"
                    ...
                    {entry.fileName && (
                      <span style={{ marginLeft: '10px' }}>{entry.fileName}</span>
                    )}
                  </td>
                </tr>
              ))
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    required
                  />
                  <small>* Unggah draft dalam format Word/PDF (maksimal 1000 MB)</small>
                </div>
